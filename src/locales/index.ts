...
export const i18n: Record<TLang, any> = {
  ru: {
    ...
    status: { longWait: "⏳ AI думает чуть дольше обычного... спасибо за терпение!" },
  ...
  tj: {
    ...
    greeting: (name: string) =>
      `Салом, ${name}\\! 👋\nМан *AI Tone Tuner* ҳастам — таҳриргари рӯҳияи матни ту\\. 💫\nМатнро навис, услуб/тонро интихоб кун — ман онро равшантар мекунам\\!\nМисол: \n _\`\React-барномасоз лозим аст\`\_`,

    share: "Мубодила",
    invite: "Ссылкаро ба дӯстон фирист ва +2 кӯшиш гир!",

    tones: {
      header: "Услуб/тонро интихоб кунед:",
      list: {
        business: "💼 Расмӣ",
        friendly: "💬 Дӯстона",
        hype: "🚀 Хайпдор",
        ...
        humorous: "😄 Бо ҳазл",
        custom: "✏️ Услуб/тони худ",
        more: "➕ Бештар",
        less: "⬅️ Камтар",
      },
    },

    status: { longWait: "⏳ AI каме бештар фикр карда истодааст... каме интизор шавед!" },

    result: {
      prefix: (toneName: string) => `Матни ту дар услуб/тони *${toneName}*:`,
      attempts: (used: number, total: number) =>
    ...
    premium: {
      offer: (premiumUrl: string) => {
        const base =
          "✨ Безлимити услуб/тон ивазкуниро фаъол намо\n\n" +
          "💎 *AI Tone Tuner Premium* барои 30 рӯз фаъол кун ва бе маҳдудият истифода бар\n\n";
        return (
          base +
          (premiumUrl.includes("localhost") || premiumUrl.includes("127.0.0.1")
            ? `\nСсылкаи пардохт: ${premiumUrl}`
            : "")
        );
      },
      alreadyHas: (until?: string) =>
    ...
    limit: {
      reached: (premiumUrl: string) => {
        const base =
          ...
          "💎 Бе маҳдудият мехоҳӣ? Premium-ро фаъол кун ✨";
        return (
          base +
          (premiumUrl.includes("localhost") || premiumUrl.includes("127.0.0.1")
            ? `\nСсылкаи пардохт: ${premiumUrl}`
    ...
    referral: {
      friendJoined: (friendName: string) =>
        `🎉 Дӯсти ту ${friendName} бо ссылкаи ту пайваст шуд!\nТу +2 кӯшиши иловаги барои имрӯз гирифтӣ 💪`,
    },

    errors: {
      somethingWentWrong: "⚠️ Хатои техники шуд. Каме баъдтар такрор кун!",
      sendTextFirst: "Аввал матнро равон кун 🙂",
      sendTextThenStyle: "Аввал матнро равон кун, баъд услуб/тонро интихоб кун 🙂",
      customTonePrompt: "Услуб/тони мехостаатро навис, (масалан: 'хайратовар')",
    },

    prompt: {
      system: (tone: string) =>
        `Ту таҳриргари профессионали ва копирайтер ҳастӣ. Матни додашударо дар ${tone} бинавис.\nҲатман:\n- Матнро осонхон бикун.\n- Қисмҳои сабуки эҳсосот ва ифодавиро илова кун, ки ба услуб/тон мувофиқанд.\n- Агар матн кӯтоҳ аст (1–2 ҷумла), онро каме зиндатар кун, вале аз ҳад зиёд ороиш макун.`,
      user: "Матн барои иваз кардани услуб/тон:",
  ...
  uz: {
    ...
    status: {
      longWait: "⏳ AI odatdagidan biroz uzoqroq o'ylayapti... sal kutib turing!",
    },
    ...
    prompt: {
      system: (tone: string) =>
        `Siz professional muharrir va kopiraytersiz. Berilgan matnni ${tone} uslubida qayta yozing.\nMajburiy:\n- Matnni tabiiy va oson o'qiladigan qiling.\n- Uslubga mos keladigan yengil hissiyot va ifodalanish qismlarini qo'shing.\n- Agar matn qisqa bo'lsa (1–2 gap), uni biroz jonliroq qiling, lekin haddan tashqari bezatmaslik.`,
      user: "Qayta yozish uchun matn:",
    },
  },
  ...
  kz: {
    ...
    status: { longWait: "⏳ AI әдеттегіден сәл ұзақ ойлануда... аздап күте тұрыңыз!" },
    ...
    prompt: {
      system: (tone: string) =>
        `Сіз кәсіби редактор және копирайтерсіз. Берілген мәтінді ${tone} стилінде қайта жазыңыз.\nМіндетті:\n- Мәтінді табиғи және оңай оқылатын етіңіз.\n- Стильге сәйкес келетін жеңіл эмоциялар мен өрнектелу бөліктерін қосыңыз.\n- Егер мәтін қысқа болса (1–2 сөйлем), оны сәл тірірек етіңіз, бірақ шектен тыс безендірмеңіз.`,
      user: "Қайта жазу үшін мәтін:",
    },
  },
};
